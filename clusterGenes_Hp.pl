...
#our $STRAIN_PRIORITY_1 = 'CGSHi3655';
#our $STRAIN_PRIORITY_2 = 'CGSSp18BS74';
#our $STRAIN_PRIORITY_3 = 'RefSpINV200';
#our $STRAIN_PRIORITY_4 = '';

our $STRAIN_PRIORITY_1 = 'HP';
our $STRAIN_PRIORITY_2 = 'HPG27';
our $STRAIN_PRIORITY_3 = 'HPP12';
our $STRAIN_PRIORITY_4 = 'HPAG1';

# list the strain names here:
our @STRAIN_LIST = qw (HPG27 HP HPP12 HPAG1 jhp HPPC HPSH HPSJM);

# alignment options
our $GENERATE_ALIGNMENTS = 0;                
our $ALIGNMENT_INFILE  = 'sequences.fa';
our $ALIGNMENT_OUTFILE = 'sequences.aln';
our $ALIGNMENT_BINARY  = 'poa';
#our $ALIGNMENT_OPTIONS = "blosum80.mat -tolower -read_fasta $ALIGNMENT_INFILE -pir $ALIGNMENT_OUTFILE";
our $ALIGNMENT_OPTIONS = "blosum80_poa.mat -tolower -read_fasta $ALIGNMENT_INFILE -pir $ALIGNMENT_OUTFILE";
...
# load ssearch matches
print STDERR "${step}: loading ssearch matches..\n";  $step++;
#load_ssearch( $ssearch_file, $GENE_PAIR_MATCHES );
...
sub load_tfasty
{
	...
	while ( my $line_input = <TFASTY> )
	{
		...
		if ( $line_input =~ /^\s*\d+>>>/ )
		{
			...
			#my ($gene1_name) = $line_input =~ />>>([A-Za-z0-9]+_\d+_c?\d+-\d+)/;
			my ($gene1_name) = $line_input =~ />>>([A-Za-z0-9]+_\d+)/;
			#my ($strain, $locus, $length) = $line_input =~ />>>([A-Za-z0-9]+)_(\d+)_c?\d+-\d+ - (\d+) aa/;
			#my ($strain, $locus, $length) = $line_input =~ />>>([A-Za-z0-9]+)_(\d+)_c?\d+-\d+\s+-?\s*(\d+) aa/;
			my ($strain, $locus, $length) = $line_input =~ />>>([A-Za-z0-9]+)_(\d+) \| .+- (\d+) aa$/; 
			...
			while ( $line_input = <TFASTY> ) 
			{
				...
				#my ($gene2_name) = $header =~ /^([A-Za-z0-9]+_\d+_c?\d+-\d+)/;
...
sub load_fasta
{
	...
	while ( my $line_input = <FASTA> )
	{
		...
		{
			# we found a record.  get strain and locus.
			#my ($gene_name) = $line_input =~ />>>([A-Za-z0-9]+_\d+_c?\d+-\d+)/;
			my ($gene_name) = $line_input =~ />>>([A-Za-z0-9]+_\d+)/;
			#my ($strain, $locus, $length) = $line_input =~ />>>([A-Za-z0-9]+)_(\d+)_c?\d+-\d+ - (\d+) nt/;
			#my ($strain, $locus, $length) = $line_input =~ />>>([A-Za-z0-9]+)_(\d+)_c?\d+-\d+\s+-?\s*(\d+) nt/;
			my ($strain, $locus, $length) = $line_input =~ />>>([A-Za-z0-9]+)_(\d+) \| .+ - (\d+) nt/; 

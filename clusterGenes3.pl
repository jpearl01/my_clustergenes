	...
	while ( my $line_input = <TFASTY> )
	{
		...
		if ( $line_input =~ /^\s*\d+>>>/ )
		{
			...
			while ( $line_input = <TFASTY> ) 
			{
				...
				last if ( $line_input =~ /^>>><<</ );
				next if ( $line_input =~ /^\+-/ );
				if ( $line_input =~ /\+-/ ){print "Line beginning prob tfasty: $line_input\n";next;};
				chomp $line_input;
				...
				if (!(defined $gene2_name || defined $gene1_name)){print "Gene name matching problem at line $. in tfasty which is:\n$line_input\n";}
...
#			print "$length\n";
			...
			while ( $line_input = <FASTA> ) 
			{
				...
				if (!(defined $gene_name || defined $match_strain)){print "Gene name matching problem at line $. in fasta which is:\n$line_input\n";}
...
sub add_gene_to_cluster
{
	...
	foreach my $gene_ref ( @_ )
	{
		...
		# add strain
		my ($strain) = $gene_ref->name =~ /^([A-Za-z0-9]+)_/ or print "Problem match ".$gene_ref->name."\n";
...
sub name_cluster
{
	...
	foreach my $gene_ref ( $cluster_ref->{genes}->getAllRefs )
	{
		...
		print "Problem match to $gene_name\n" unless $strain;
